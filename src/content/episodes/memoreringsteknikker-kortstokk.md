---
title: "Masterclass: Memoreringsteknikker – lær å memorere en kortstokk"
format: masterclass
status: publisert
publishDate: 2026-08-07
categories:
  - Utdanning
topics:
  - Husketeknikk og memorering
  - Minnepalass (loci-metoden)
  - Bildekoding av kort
  - PAO-systemet (person, handling, objekt)
  - Trening og progresjon
  - Overføring til hverdag og studier
questions:
  - Hvordan klarer noen å huske en hel kortstokk i riktig rekkefølge?
  - Hva er et minnepalass, og hvordan bygger man et?
  - Hvordan gjør man om et spillkort til et bilde man husker?
  - Hva er PAO-systemet, og hvorfor gjør det memorering raskere?
  - Hvor lang tid tar det å lære seg dette?
  - Kan teknikkene brukes til noe annet enn kortstokker?
takeaways:
  - Å huske en kortstokk handler ikke om talent, men om et system du kan lære.
  - Hjernen husker bilder og steder langt bedre enn abstrakte symboler.
  - Med minnepalass kobler du informasjon til steder du allerede kjenner godt.
  - De samme teknikkene fungerer på navn, tall, fagstoff og presentasjoner.
duration: "Cirka 45 min"
description: >-
  Hvordan klarer noen å memorere en hel kortstokk? En praktisk Masterclass om
  husketeknikk: minnepalass, bildekoding og systemene som gjør det mulig – og
  hvordan du kan bruke dem selv.
image: /images/episoder/memorering-masterclass.jpg
imageAlt: "Spørretimen Masterclass: Memoreringsteknikker – lær å memorere en kortstokk"
links:
  youtube: https://youtu.be/JMAWf7hJT-o
  spotify: https://open.spotify.com/episode/6NKXd3VkGM9MKhIySnpAtC
featured: true
related:
  - memorering-kort-forklart
---

Hvordan er det mulig å huske 52 kort i riktig rekkefølge? Svaret er verken talent
eller fotografisk hukommelse – det er et **system**, og det kan læres.

I denne Masterclass-episoden går Jan Sindre gjennom memoreringsteknikkene som ligger
bak: hvordan du bygger et **minnepalass**, hvordan hvert kort gjøres om til et
tydelig bilde, og hvordan disse bildene plasseres langs en rute du allerede kjenner.
Vi ser også på **PAO-systemet**, der hvert kort får en person, en handling og et
objekt – slik at flere kort kan pakkes sammen til ett minne.

Poenget er ikke kortstokken i seg selv. Det er å forstå hvorfor hjernen husker
bilder og steder så mye bedre enn tall og symboler – og hvordan du kan bruke det
samme trikset på navn, fagstoff, handlelister og presentasjoner.

*Innholdet er praktisk kunnskap du kan øve på selv. Kilder og videre lesing oppgis
i episoden.*
